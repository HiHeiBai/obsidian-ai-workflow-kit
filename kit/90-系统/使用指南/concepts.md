...
`00-入口/开始这里.md` tells the agent what kind of task it is handling, which files to read first, and where results should be written.

### 2. Knowledge Pipeline

`20-资料/处理流程/` tells AI how to intake local materials, classify them, extract value, connect them, promote reusable knowledge, and maintain the result.

### 3. Agent Governance

`90-系统/规则/` tells AI what it may write, what it must verify, and when it should stop instead of guessing.

### 4. Recall System

`90-系统/召回/` maps tasks to the files AI should read first.
...
```text
10-项目/01-示例项目/BRIDGE-example.md
```

### 6. Inbox Is Temporary

`01-收件箱/` is for incoming handoffs, task cards, and web clips. It is not a permanent storage area.

The current conversation is the default execution context. Create `01-收件箱/任务/` cards only when work is queued, crosses sessions, waits on an external condition, or needs explicit coordination. Add specialist review perspectives directly to the task when needed.

### 7. Lessons Become Assets

Repeated lessons should move into `30-经验资产/` with a clear trigger, action, and verification method.
...
```text
90-系统/示例/ai-handoff-demo/README.md
```

For a more realistic filled example, see:

```text
90-系统/示例/filled-example/
```

For a source-to-knowledge example, see:

```text
90-系统/示例/source-to-knowledge/
```

Daily use should stay small: after the first setup, most agent sessions only need `00-入口/开始这里.md`, one project bridge card, and that project's `current-state.md` / `decisions.md`.
...
| Path | Purpose |
|---|---|
| [`00-AI/START-HERE.md`](../../00-入口/开始这里.md) | The first file an AI agent should read |
| [`index.md`](../../首页.md) | Human-facing vault homepage |
| [`00-AI/AGENTS.md`](../AI协作规则.md) | Rules for AI agents such as Claude Code, Cursor, and Codex |
| [`00-AI/governance/`](../规则) | Startup contract, review gates, write-back rules, maintenance loop |
| [`01-Inbox/`](../../01-收件箱) | Temporary handoffs, queued/cross-session task cards, and web clips |
| [`00-AI/pipeline/`](../../20-资料/处理流程) | How AI turns local materials into structured knowledge |
| [`00-AI/recall/`](../召回) | Task-to-context maps and recall fields |
| [`00-AI/bases/`](../视图) | Optional project, task, and source views installed in full mode |
| [`10-Projects/`](../../10-项目) | Project workspaces and bridge cards |
| [`20-SharedAssets/`](../../30-经验资产) | Reusable methods, SOPs, and workflows |
| [`40-ExternalSources/`](../../20-资料) | Source analysis cards, not copied third-party articles |
| [`00-AI/templates/`](../模板) | Standard templates |
| [`docs/`](.) | First-run guides, diagrams, and user-facing walkthroughs |
| [`00-AI/scripts/`](../脚本) | Small helper scripts for project cards and health checks |
| [`examples/`](../示例) | End-to-end handoff demo |
