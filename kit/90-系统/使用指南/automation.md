...
3. Let the agent write handoffs and project updates by following `00-入口/开始这里.md`.
...
```bash
python3 90-系统/脚本/kb.py stale-check \
...
```text
90-系统/示例/claude-code-hooks/
```

Basic setup:

1. In your vault, create `.claude/hooks/`.
2. Copy `90-系统/示例/claude-code-hooks/stop-session-check.py` to `.claude/hooks/stop-session-check.py`.
3. Merge `90-系统/示例/claude-code-hooks/settings.example.json` into `.claude/settings.json`.
