...
`--update` upgrades managed kit files from the latest GitHub version. It uses `90-系统/配置/kit-manifest.json` (or the legacy root manifest for established shared-core consumers) to tell whether a file is still the original kit file or has been changed by the user.
...
- Records managed kit files in `90-系统/配置/kit-manifest.json` for new full/barebone installs so future updates can be safer.
