...
复制下列字段和章节建立卡片。规则见 `90-系统/规则/写回规则.md`；本卡保存当前摘要，逐项替换，不按日期追加。无状态、决策、边界、证据或下一步变化时不写。
...
- <已沉淀经验链接；有复用经验写入 `30-经验资产/06-本地经验/`，不要写进 managed `02-modules/`。没有则删除。>
...
- 更新后执行只读检查：`python3 90-系统/脚本/kb.py check-record <本卡路径>`。
