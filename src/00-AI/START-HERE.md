...
- 有项目任务时，只读对应项目桥接卡。
- 分析外部资料时先读资料本身；需要落库时再读资料分析模板和相关项目页。
- 不要一上来扫描整个 vault。
...
| 问题/事故经验 | `00-AI/templates/TPL-incident-experience-card.md` 复制后写入对应项目或 `20-SharedAssets/01-user-assets/` |
| 召回规则 | `00-AI/recall/` |
...
- 项目事实是否变化；有变化才替换桥接卡当前摘要，按 `00-AI/governance/write-back-rules.md` 控制长度并保全证据，无变化不写。
