...
已知项目或目标文件时，直接读取相关项目约定和事实文档。需要定位入口或了解库结构时，再读 `00-AI/START-HERE.md` 或 `index.md`；治理规则和模板只在相应操作发生时加载。已读且仍有效的内容不重复加载。
...
- 项目状态变化时按 `00-AI/governance/write-back-rules.md` 更新事实文档与桥接摘要；卡片长度、动作镜像、证据保全和写后检查只在该页定义。
...
- 经判断值得长期保留且不与已有内容重复的经验，写到用户所有的 `20-SharedAssets/01-user-assets/`；`02-modules/` 只保存 kit 维护的通用机制和标准。
...
- 只有确实需要另一个窗口或 Agent 接手时才写 `01-Inbox/agent-handoffs/`，按 `00-AI/governance/write-back-rules.md` 执行；文件变化或任务完成本身不触发交接卡。
